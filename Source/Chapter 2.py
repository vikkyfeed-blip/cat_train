...
def sample_data_calculate(df, n):
    sample_d = pd.DataFrame({
        'data': [df.sample(n).mean() for _ in range(1000)],
        'type': f'mean of {n}',
    })
    return sample_d

def plothist(df):
    g = sns.FacetGrid(df, col='type', col_wrap=1,
                      height=2, aspect=2)
...
def load_file(file_name):
    ...
    project_roof = os.path.dirname(current)
    file_path = os.path.join(project_roof, 'Data', file_name)
    data = pd.read_csv(str(file_path))
    return data

if __name__ == '__main__':
    loans_income = load_file('loans_income.csv')
    sample_data = pd.DataFrame({
        'data': (loans_income.sample(1000)).values.flatten(),
        'type': 'Data',
    })
    sample_data_05 = sample_data_calculate(sample_data.iloc[:,0], 5)
    sample_data_20 = sample_data_calculate(sample_data.iloc[:,0], 20)
